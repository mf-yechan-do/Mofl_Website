...
class UpdateContent extends Component {
  render() {
    const { store } = this.props;
      ...
      const contentobj = store.content_id;
const onChange = (event) => {
  store.content[1].desc = event.currentTarget.value;
 debugger;
}
  ...
  let textcontent = "";
   while (k < store.content.length) {
    ...
    event.preventDefault();
          ...
          <p>
            <input type="text" id="name"  onChange={content} value={textcontent}/>
          </p>
          <p>
            <textarea name="desc" id="desc"  onChange={content2} value={desccontent}    />
          </p>
          <p>
            <input
              type="submit"
